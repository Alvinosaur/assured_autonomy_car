...
- What happens when a map update happens and the original plan needs to be revised?
We simply update any transition costs, which are stored in a separate datastructure. We then find the best neighbor of the current start state
(robot's current state), best being the one with minimum G-value + updated transition costs. We set the start state's V-value equal to this min G + trans_cost since the V-value should reflect a more up-to-date version of the G value. Then we add the current start state back to the open set and let it get expanded when its turn comes up. When it gets expanded, its G-value will not be equal to its V-value, meaning it is inconsistent and overall G < V. Since G should always be >= V, we need to set current start state's G-value = infinity. Then we propagate the G=infinity to all of  start state's neighbors by updating their G-values and reinserting them into the open-set. This way, the changes gets propagated, and every other state gets updated by the original transition cost changes. Now, overall a new best path will be generatedd, but the difference with Astar is that we don't specify that every state needs to be re-expanded. Dstar selectively re-expands states in a greedy fashion, and overall may not re-expand all states, but only the necessary states to find a new path.
